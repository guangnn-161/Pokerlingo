...
export async function PATCH(request: Request) {
  if (!requireSameOrigin(request)) return fail(request, 403, "ORIGIN_FORBIDDEN", "Nguồn yêu cầu không hợp lệ.");
  const user = await getCurrentUser();
  if (!user) return fail(request, 401, "UNAUTHENTICATED", "Bạn cần đăng nhập.");
  const limit = takeRateLimit(`profile:${user.id}`, 10);
  if (!limit.allowed) return fail(request, 429, "RATE_LIMITED", "Bạn cập nhật hồ sơ quá nhiều lần. Vui lòng thử lại sau.");
  const parsed = profileUpdateSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return fail(request, 400, "VALIDATION_ERROR", "Thông tin hồ sơ không hợp lệ.", { fields: parsed.error.flatten().fieldErrors });
